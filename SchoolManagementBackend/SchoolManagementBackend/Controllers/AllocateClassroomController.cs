...
namespace SchoolManagementBackend.Controllers
{
    ...
    public class AllocateClassroomController : ControllerBase
    {
        ...
        [HttpGet("GetAllocateClassroomByID/{id}")]
        public async Task<ActionResult> GetAllocateClassroomByID(int id)
        {
            try
            {
                var result = await _allocateClassroomService.GetAllocateClassroomByID(id);
                if (result.Success)
                    return Ok(result);
                else
                {
